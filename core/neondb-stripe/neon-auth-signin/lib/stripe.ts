...
// singleton prevents deploy errors when we don't have the key
// mostly just useful for shipping the template
...
export async function processStripeEvent({ body, signature }: { body: string; signature: string }) {
  const { event, success: eventSuccess, error: eventError } = getStripeWebhookEvent({ body, signature })
...
function isAllowedEventType<TEvent extends Stripe.Event>(event: TEvent): event is TEvent & { type: AllowedEventType } {
